...
validate(mtcars) %>%
  ...
  assert_cols(description = "For wt and qsec we have: abs(col) < 4 * sd(col)", predicate_generator = within_n_sds(4), wt, qsec) %>%
  assert_cols(description = "For wt and qsec we have: abs(col) < 2 * sd(col)", predicate_generator = within_n_sds(2), wt, qsec) %>%
